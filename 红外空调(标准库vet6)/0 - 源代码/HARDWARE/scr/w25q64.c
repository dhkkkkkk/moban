...
{  
	u8 byte=0;                            
	W25Q64_CS=0;                              //使能器件   
	SPI1_ReadWriteByte(W25Q64_ReadStatusReg);    //发送读取状态寄存器命令    
...
{   
	W25Q64_CS=0;                              //使能器件   
	SPI1_ReadWriteByte(W25Q64_WriteStatusReg);   //发送写取状态寄存器命令    
...
{
	W25Q64_CS=0;                            //使能器件   
    SPI1_ReadWriteByte(W25Q64_WriteEnable);    //发送写使能  
	W25Q64_CS=1;                            //取消片选     	      
} 
...
{  
	W25Q64_CS=0;                            //使能器件   
    SPI1_ReadWriteByte(W25Q64_WriteDisable);   //发送写禁止指令    
	W25Q64_CS=1;                            //取消片选     	      
} 	
...
{ 
 	u16 i;   										    
	W25Q64_CS=0;                              //使能器件   
    SPI1_ReadWriteByte(W25Q64_ReadData);         //发送读取命令   
...
void W25Q64_Write_Page(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)
{
	...
	W25Q64_CS=0;                              //使能器件   
    SPI1_ReadWriteByte(W25Q64_PageProgram);      //发送写页命令   
...
{                                   
    ...
    W25Q64_Wait_Busy();                     //等待空闲
  	W25Q64_CS=0;                            //使能器件   
    SPI1_ReadWriteByte(W25Q64_ChipErase);      //发送片擦除命令  
	W25Q64_CS=1;                            //取消片选     	      
...
{  	  
    ...
    W25Q64_Wait_Busy();                       //等待空闲  
  	W25Q64_CS=0;                              //使能器件   
    SPI1_ReadWriteByte(W25Q64_SectorErase);      //发送扇区擦除指令 
