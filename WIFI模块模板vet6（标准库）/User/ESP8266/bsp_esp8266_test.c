...
void ESP8266_StaTcpServer_ConfigTest(void)
{
  ...
  while( ! ESP8266_BuildAP(macUser_ESP8266_ApSsid, macUser_ESP8266_ApPwd,OPEN));	
...
//    printf ( "\r\n%s\r\n", strEsp8266_Fram_Record .Data_RX_BUF );//
    ...
    if ( ( pCh = strstr ( strEsp8266_Fram_Record .Data_RX_BUF, "CMD_LED_" ) ) != 0 )    //LED控制  直接更改为*******?*?
    {
      cCh = * ( pCh + 8 );  //地址+8位，对应CMD_LED_?
      printf("%c\n",cCh);
      switch ( cCh )  //第八位
      {
        case '1':
          cCh = * ( pCh + 10 );  //地址+10位，对应CMD_LED_*_?
          switch ( cCh )  //第十位
          {
            case '0':
              //macLED1_OFF();
						GPIO_SetBits ( an1_GPIO_PORT, an1_GPIO_PIN );
						GPIO_ResetBits ( an2_GPIO_PORT, an2_GPIO_PIN );
              ucLed1Status = 0;
              break;
            case '1':
              //macLED1_ON();
						GPIO_SetBits ( an2_GPIO_PORT, an2_GPIO_PIN );
						GPIO_ResetBits ( an1_GPIO_PORT, an1_GPIO_PIN );
              ucLed1Status = 1;
              break;
            default :
              break;
